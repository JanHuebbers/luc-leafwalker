...
- [Environment setup](#environment-setup)
  - [Fonts on WSL](#fonts-on-wsl)
- [Workflow](#workflow)
  - [1. Create input `.xlsx` files](#1-create-input-xlsx-files)
  - [2. Normalize luciferase data](#2-normalize-luciferase-data)
  - [3. Run downstream analysis](#3-run-downstream-analysis)
...
3. **Reference-based normalization**
   Baseline-corrected values are divided by the mean value of a specified reference sample within the same replicate. This yields normalized values that allow comparison across samples and experiments.
4. The processed data are appended to a long-format `.csv` table.
#### Example:
```bash
